...
# (rete assente durante l'OTA) si esce lasciando la sessione X11 al suo posto,
# e il prossimo OTA riproverà.
...
# /run/hifi-player: la sessione gira come utente hifi e /run è di root, quindi
# senza questa regola il modo nativo del pannello non sarebbe memorizzabile e
# hifi-ui-resolution.sh perderebbe l'idempotenza dopo il primo cambio di modo.
...
# Autologin: dalla sessione X11 a quella Wayland. Solo se il file esiste ed è
# ancora puntato alla sessione X11 — se l'utente/una migrazione futura l'ha
# cambiato a mano non lo si sovrascrive.
if [ -f "$LIGHTDM_CONF" ] && grep -q '^user-session=hifi-kiosk$' "$LIGHTDM_CONF"; then
    if [ -x /usr/local/bin/hifi-kiosk-wayland ] && command -v labwc >/dev/null 2>&1; then
        backup_and_edit "$LIGHTDM_CONF" "" 's/^user-session=hifi-kiosk$/user-session=hifi-kiosk-wayland/' || true
    fi
fi

# La sessione cambia solo al prossimo login: se abbiamo toccato qualcosa serve
# un riavvio per applicarla davvero.
