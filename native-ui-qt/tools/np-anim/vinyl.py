...
flat textures (the label and the strobe dots turn, the tone arm swings).
  ...
  vinyl.py [OUT_DIR] [--only-plinth]      default: native-ui-qt/assets/anim/vinyl/
...
Layers, bottom to top, as AnimVinyl.qml stacks them:
  ...
  plinth.png         black 90s satin plinth, arm board, pivot base, arm rest,
                     cue lever, anti-skate knob, start/stop and speed buttons,
                     the LED (off), the pitch fader, the prints, the engraved
                     name plate and the platter's shadow
...
# the 90s plinth: a pitch fader on the right, prints
...
def main():
    ...
    fns = (render_plinth_shadow, render_plinth, render_platter, render_dots,
           render_record_shadow, render_record, render_label, render_label_shade, render_spindle, render_arm,
           render_arm_shadow, render_led)
    # --only-plinth: just plinth.png (the others take about a minute)
    if "--only-plinth" in sys.argv:
        fns = (render_plinth,)
    for fn in fns:
