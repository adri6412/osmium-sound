...
Item {
    ...
    component QrCorner: Rectangle {
        ...
        x: root.width - 16 - width; y: 64; width: 124; height: 124 + 20; radius: 12; color: Theme.white
    ...
    Item {
        ...
        Item {
            ...
            readonly property real ty0: Math.max(16, root.height / 2 - bh / 2)
            Hero { x: root.width / 2 - 32; y: parent.ty0; side: 64; iconPx: 32 }
            ...
            Text { id: setupSub; x: root.width / 2 - 220; y: parent.ty0 + 126; width: 440; height: 40; wrapMode: Text.Wrap; maximumLineCount: 2; horizontalAlignment: Text.AlignHCenter; text: Tr.t(root.connected ? "wizard.qr.connectedSubtitle" : "wizard.wifi.setupSubtitle"); color: Theme.silverA(0.7); font.family: Theme.font; font.pixelSize: 14 }
            ...
            Item {
                visible: !root.connected
                x: root.width / 2 - 190; y: parent.ty0 + 182; width: 380
                readonly property real formH: root.pick >= 0 ? 12 + 44 + 8 + 40 : 32
                readonly property real listH: Math.max(44, Math.min(root.nets.length * 44, root.height - 16 - y - formH - 20))
            ...
            Text { visible: root.wifiErr !== "" || (root.perror !== "" && root.stage === "failed"); x: root.width / 2 - 190; y: root.height - 38; width: 380; height: 24; horizontalAlignment: Text.AlignHCenter; verticalAlignment: Text.AlignVCenter; text: root.wifiErr || root.perror; color: Theme.red400; font.family: Theme.font; font.pixelSize: 12 }
        }
        // ── installer ───────────────────────────────────────────────────────
        Item {
            visible: root.mode === 2
            ...
            Item {
                visible: parent.busy
                anchors.fill: parent
                readonly property real y0: root.height / 2 - (64 + 24 + 26 + 4 + 40 + 24 + 8) / 2
                Hero { x: root.width / 2 - 32; y: parent.y0; side: 64; iconPx: 32 }
                Icon { visible: parent.parent.done; x: root.width / 2 - 16; y: parent.y0 + 16; name: "check-circle"; size: 32; color: Theme.black }
                ...
                Text { x: root.width / 2 - 192; y: parent.y0 + 118; width: 384; height: 40; wrapMode: Text.Wrap; maximumLineCount: 2; horizontalAlignment: Text.AlignHCenter
                ...
                Rectangle { visible: parent.parent.running; x: root.width / 2 - 192; y: parent.y0 + 178; width: 384; height: 8; radius: 4; color: Theme.border
                            Rectangle { width: parent.width * Math.max(0, Math.min(100, root.progress)) / 100; height: 8; radius: 4; color: Theme.gold } }
                Text { visible: parent.parent.done && root.countdown > 0; x: root.width / 2 - 160; y: parent.y0 + 174; width: 320; height: 18; horizontalAlignment: Text.AlignHCenter; text: "Rebooting in " + root.countdown + "s…"; color: Theme.silverA(0.5); font.family: Theme.font; font.pixelSize: 12 }
                BigButton { visible: parent.parent.err; x: root.width / 2 - 60; y: parent.y0 + 166; width: 120; height: 40; color: Theme.gold; label: "Retry"; onClicked: { root.state = "idle"; root.stepTo(0) } }
            }
            // welcome
            Item {
                visible: !parent.busy && root.step === 0
                anchors.fill: parent
                readonly property real y0: root.height / 2 - (80 + 24 + 36 + 12 + 48 + 32 + 46 + 24 + 18) / 2
                Hero { x: root.width / 2 - 40; y: parent.y0; side: 80; iconPx: 40 }
                ...
                Text { x: root.width / 2 - 224; y: parent.y0 + 152; width: 448; height: 48; wrapMode: Text.Wrap; maximumLineCount: 2; horizontalAlignment: Text.AlignHCenter; text: "This will install Osmium Sound onto this computer's disk. All data on the chosen disk will be erased."; color: Theme.silverA(0.7); font.family: Theme.font; font.pixelSize: 15 }
                BigButton { x: root.width / 2 - 105; y: parent.y0 + 232; width: 210; color: Theme.gold; label: "Choose disk"; trail: "chevron-right"; onClicked: root.stepTo(1) }
                Text { x: root.width / 2 - 240; y: parent.y0 + 302; width: 480; height: 18; horizontalAlignment: Text.AlignHCenter; text: "Or scan the QR code (top right) to use your phone instead"; color: Theme.silverA(0.5); font.family: Theme.font; font.pixelSize: 12 }
            }
            // scelta del disco
            Item {
                ...
                readonly property real y0: 48 + (root.height - 48 - 56) / 2 - (30 + 4 + 20 + 32 + listH) / 2
                ...
                Column {
                    visible: !root.disksLoading && root.disks.length > 0
                    x: root.width / 2 - 256; y: parent.y0 + 86; spacing: 12
                    Repeater {
                        model: root.disks
                        Rectangle {
                            required property var modelData
            ...
            Item {
                ...
                anchors.fill: parent
                readonly property real y0: 48 + (root.height - 48 - 56) / 2 - (40 + 16 + 32 + 16 + 92 + 24 + 46) / 2
                readonly property var dk: root.sel >= 0 && root.sel < root.disks.length ? root.disks[root.sel] : { path: "", model: "" }
                Icon { x: root.width / 2 - 20; y: parent.y0; name: "alert-circle"; size: 40; color: "#fbbf24" }
                ...
                Rectangle { x: root.width / 2 - 256; y: parent.y0 + 104; width: 512; height: 92; radius: 16; color: Qt.rgba(0x78/255, 0x35/255, 0x0f/255, 0.1); border.width: 1; border.color: Qt.rgba(0xf5/255, 0x9e/255, 0x0b/255, 0.3)
                ...
                BigButton { x: root.width / 2 - 256; y: parent.y0 + 220; width: 512; color: "#d97706"; fg: Theme.white; label: "Erase and install"
