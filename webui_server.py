...
function loadRemoteStep(){
  ...
  jget('/api/system/bt_remotes').then(function(r){
    ...
    // the USB half of the step still stands, so the step stays.
    var on=!!(r&&r.available&&r.supported);
    byId('btn-remote-scan').style.display=on?'block':'none';
    byId('remote-bt-hint').textContent=on?S.remoteBtHint:
      (r&&r.available?S.remoteNeedsUpdate:S.remoteNoBt);
  });
}
function remotePick(){
