...
function loadRemoteStep(){
  ...
  jget('/api/provision/bt_remotes').then(function(r){
    // no radio on this box, or an image whose supervisor predates remotes:
    // the USB half of the step still stands, so say so instead of showing a
    // scan button that cannot work
    var on=!!(r&&r.available&&r.supported);
    byId('btn-remote-scan').style.display=on?'block':'none';
    byId('remote-bt-hint').textContent=on?S.remoteBtHint:S.remoteNoBt;
  });
}
function remotePick(){
...
function remotePair(mac,btn){
  ...
  jpost('/api/provision/bt_remotes/add',{mac:mac}).then(function(r){
    ...
    // so the picker is refreshed again after a beat, not just right now
    loadRemoteStep();
    setTimeout(loadRemoteStep,4000);
