 ...
 * (the ONLY way to do the network step on-screen now -- there is no setup
 * hotspot any more, see SetupWizard.jsx) and Settings' post-setup Network
 * section. `networks`, `scanning`, `connecting` and `error` are all driven
 * by the parent — this component only collects ssid/password and hands off
 * to `onConnect`.
 *
 * `inline`: SetupWizard renders this as its main content (no backdrop, no
 * close button — there's nothing else to fall back to before the network
 * step). Settings keeps the default modal presentation, opened/closed by
 * its own button.
 */
const WifiConfigPanel = ({ networks, scanning, connecting, error, onConnect, onClose, inline = false }) => {
  const { t } = useI18n();
